...
class ElfinderConnector:
    ...
    _commands = {
        ...
        'upload': {'target': True, 'FILES': True, 'mimes': False, 'html': False, 'upload_path': False, 'chunk': False,
                   'range': False, 'cid': False},
        'get': {'target': True},
                ...
                for file_index in all_[item]:
                    try:
                        ...
                        if len(all_[item]) >= 1 and isinstance(upload_path, list) and target not in upload_path:
                            ...
                            if chunk:
                                file_ = volume.upload(files[file_index], target, chunk=True,
                                                      first_chunk=chunk_flag)  # This is a weird bug
                                file_.update({'size': chunk_file_size})
                            else:
                                file_ = volume.upload(files[file_index], target)
                        result['added'].append(file_)
                    except Exception as e:
                        result['warning'] = self.error(ElfinderErrorMessages.ERROR_UPLOAD_FILE, files[file_index].name,
                                                       e)
                        self._uploadDebug = 'Upload error: Django handler error'
        return result
